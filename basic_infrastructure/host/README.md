...
This guide covers the Ansible-based host provisioning setup used in `basic_infrastructure/host`.

## Overview

Ansible is used to provision and configure the target server for a single Starter Architecture project. It uses an inventory file to define the hosts and connection details required to run playbooks over SSH.
...
From the `basic_infrastructure/host` directory, you can verify the setup with the following commands.
...
To provision the server, run the main playbook from the `basic_infrastructure/host` directory:
